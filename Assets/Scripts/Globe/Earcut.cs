...
namespace Mapbox
{
    public static class EarcutLibrary
    {
        ...
        public static Data Flatten(JArray data)
        {
            var dim = (data[0][0] as JArray).Count;
            var result = new Data() { Dim = dim };
            var holeIndex = 0;

            for (var i = 0; i < data.Count; i++)
            {
                for (var j = 0; j < (data[i] as JArray).Count; j++)
                {
                    for (var d = 0; d < dim; d++)
                        result.Vertices.Add((float)data[i][j][d]);
                }
                if (i > 0)
                {
                    holeIndex += (data[i - 1] as JArray).Count;
                    result.Holes.Add(holeIndex);
                }
            }
            return result;
